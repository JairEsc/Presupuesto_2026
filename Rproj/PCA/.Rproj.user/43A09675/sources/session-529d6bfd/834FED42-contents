...
factoextra::fviz_pca_var(pca_indicadores_full, col.var = "black")
...
(pca_indicadores_full$rotation[pca_indicadores_full$rotation[,1] |> abs() |> order(decreasing = T),1])[1:5] |> names() |> sapply(\(z){z==diccionario_variables$variables},simplify = T,USE.NAMES = F) |> which()
##Componente 1: 
pca_indicadores_full$rotation[pca_indicadores_full$rotation[,1] |> abs() |> order(decreasing = T),1] |> abs() |> plot()
pca_indicadores_full$rotation[pca_indicadores_full$rotation[,1] |> abs() |> order(decreasing = T),1]##Cortamos a 5
####-----------
